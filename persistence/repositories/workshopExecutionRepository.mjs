...
import {
  ...
  WorkshopExecution_FullDefinitionViewById
...
WorkshopExecutionRepository.findFullViewById = async function (id) {
  return findViewByCriteria(WorkshopExecution_FullDefinitionViewById, ['id', DmlOperators.EQUALS, id]);
};
...
WorkshopExecutionRepository.findEnrollmentByParticipantId = async function (id) {
  const [startOfDay] = getRangeForToday();
