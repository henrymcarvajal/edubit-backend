...
import {
  ...
  LOANS_TERMS, getAverageInstallment
} from '../loans.mjs';
...
const purchaseWithMortgage = (progresses, purchase) => {
  checkAllAssetsAreMortgageable(purchase.assets);

  const principal = totalAssetsPayment(purchase.assets);

  const { MORTGAGE: { INTEREST_RATE: rate, DOWN_PAYMENT_PERCENTAGE: downPaymentPercentage } } = LOANS_TERMS;
  ...
  registerLoan(progresses, purchase, rate, installment, FINANCIAL_SOURCES_OPTIONS.MORTGAGE);

  return averageDownPayment;
};

const checkAllAssetsAreMortgageable = (assets) => {
...
const checkAverageInstallment = (progresses, principal, rate, installments) => {
  ...
  const incomeBelowAverageInstallment = findIncomeBelowAverageInstallment(progresses, averageInstallment);
  if (incomeBelowAverageInstallment) {
    ...
    throw new ResourceStateError(PurchasesMessages.NOT_ENOUGH_INCOME_FOR_INSTALLMENT(averageInstallment, totalIncome));
  }
...
export default purchaseWithMortgage;